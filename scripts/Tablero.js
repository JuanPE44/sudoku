...
class Tablero {
        ...
        for (let c = 0; c < 3; c++) {
          for (let d = 0; d < 3; d++) {
            ...
            if (this.cajas[a][c][b][d].innerHTML !== " ") {
              this.numerosCompletos[
                parseInt(this.cajas[a][c][b][d].innerHTML) - 1
              ]++;
            }
  ...
  obtenerMejorTiempo() {
    ...
    tiempos.forEach((t) => {
      ...
      if (parseInt(min[0]) < parseInt(minutoMin)) {
        ...
        parseInt(min[0]) == parseInt(minutoMin) &&
        parseInt(min[1]) < parseInt(segundoMin)
      ) {
        segundoMin = min[1];
      }
    });
  ...
  ganarPartida() {
    ...
    const iconoErrores =
      '<div class="win-icon"><i class="fa-solid fa-xmark"></i></div>';
    const iconoTiempo =
      '<div class="win-icon"><i class="fa-regular fa-clock"></i></div>';
    const iconoMejor =
  ...
  botonRetroceder() {
    let filtrado = t.historial.filter(
      (e) =>
        e.elemento.classList.contains("incorrecto") ||
        e.elemento.classList.contains("correcto")
    );
